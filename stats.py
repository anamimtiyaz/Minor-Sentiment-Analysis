...
def printAllReductionStats(tweets):
...
tweets = stanfordcorpus.getNormalisedTweets('stanfordcorpus/'+stanfordcorpus.FULLDATA+'.5000.norm.csv')

printClassStats( tweets )
printFeaturesStats( tweets )
# printReductionStats( tweets, function, filtering=True)
printAllReductionStats(tweets)
printFreqDistCSV( dist, filename='' )
preprocessingStats( tweets, fileprefix='' )
stepStats( tweets, num_bins=10, split='easy', fileprefix='' )
oldStats2CSV( in_file, fileprefix='')
newStats2CSV(files, out_file)