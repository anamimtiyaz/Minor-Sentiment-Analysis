...
def getTrainingAndTestData(tweets, K, k, method, feature_set):
    ...
    def extract_features(words):
        ...
        if(extract_features.count <= 90000):
...
def trainAndClassify( tweets, classifier, method, feature_set, fileprefix ):
    ...
    INFO = '_'.join([str(classifier)] + [str(k) + str(v) for (k, v) in list(feature_set.items())])
    ...
    if '1step' == method:
     for k in range(FOLDS):
        ...
        matrix = confusion_matrix( test_truth, test_predict)

        precision = float(matrix[0][0]) / float(matrix[0][0] + matrix[0][1])
        recall = float(matrix[0][0]) / float(matrix[0][0] + matrix[1][0])
        print("Recall : " + str( recall ))
        print("Accuracy : " + str(float(matrix[0][0] + matrix[1][1]) / float(matrix[0][0] + matrix[1][0] + matrix[0][1] + matrix[1][1])))
     ...
     print(("\nF1Scores :", fscores))
     print(("Average F1Score :", sum(fscores) / FOLDS))
...
def main(argv) :
    ...
    if (len(argv) >= 3) :
        # print(argv[3] + '*******************************************************\n')
        ngramVals = [int(val) for val in argv[2].split(',') if val.isdigit()]
    else :
        ngramVals = [ 1 ]
    ...
    if (len(argv) >= 4) :
        # print(argv[2] + '*******************************************************\n')
        methodNames = [name for name in argv[3].split(',') if name in LIST_METHODS]
    else :
        methodNames = ['1step']
